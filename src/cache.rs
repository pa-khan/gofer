...
impl<V> CacheEntry<V> {
    ...
    pub fn new_with_mtime(value: V, size_bytes: usize, mtime: Option<std::time::SystemTime>) -> Self {
        let now = Instant::now();
        Self {
            value,
            inserted_at: now,
...
impl<K: Hash + Eq + Clone, V: Clone> LruCache<K, V> {
    ...
    pub fn put_with_mtime(&mut self, key: K, value: V, size_bytes: usize, mtime: Option<std::time::SystemTime>) {
    ...
    pub fn invalidate_prefix(&mut self, prefix: &str) where K: AsRef<str> {
        let keys_to_remove: Vec<K> = self.cache.keys()
...
impl CacheManager {
    ...
    pub async fn get_file_with_mtime(&self, path: &str) -> Option<(String, Option<std::time::SystemTime>)> {
        let mut cache = self.file_cache.write().await;
    ...
    pub async fn put_file_with_mtime(&self, path: String, content: String, mtime: Option<std::time::SystemTime>) {
    ...
    pub async fn get_stats(&self) -> CacheStats {
        ...
        stats.total_entries = stats.file_cache_entries + stats.symbol_cache_entries + stats.search_cache_entries;
        stats.total_size_bytes = stats.file_cache_size + stats.symbol_cache_size + stats.search_cache_size;
    ...
    async fn evict_expired_entries(&self) {
        ...
        let symbol_evicted = self.symbol_cache.write().await.evict_expired(self.symbol_ttl);
        let symbol_rkyv_evicted = self.symbol_cache_rkyv.write().await.evict_expired(self.symbol_ttl);
        let search_evicted = self.search_cache.write().await.evict_expired(self.search_ttl);
